...
class SubCategoriesController extends MainAdminController
{
    ...
    { 
        ...
        if($sub_category_image){
            ...
            \File::delete(public_path() .'/upload/sub_category_image/'.$subcat_obj->sub_category_image.'-b.jpg');
            \File::delete(public_path() .'/upload/sub_category_image/'.$subcat_obj->sub_category_image.'-s.jpg');
            
            $tmpFilePath = 'upload/sub_category_image/';          
             
            $hardPath = substr($sub_category_slug,0,100).'_'.time();
            
            $img = Image::make($sub_category_image);

            $img->save($tmpFilePath.$hardPath.'-b.jpg');
            
            $img->fit(300, 300)->save($tmpFilePath.$hardPath. '-s.jpg');

            $subcat_obj->sub_category_image = $hardPath;
