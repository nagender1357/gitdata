...
class ListingsUserController extends Controller
{
    ...
    { 
       ...
       $location = $inputs['country_location'];
    ...
    { 
        ...
        //$restaurants = Restaurants::orderBy('restaurant_name')->get();
        $inputs = $request->all();
		if($inputs['cate_search'])
		{
       $category = $inputs['cate_search'];
		}
		if($inputs['cate_search'] && $inputs['sub_cate_search'])
		{
	   $category = $inputs['sub_cate_search'];
		}
	   if($inputs['cate_search'] && $inputs['sub_cate_search'] && $inputs['sub_sub_cate_search'])
		{
	   $category = $inputs['sub_sub_cate_search'];
		}
	...
	public function ajax_subcatedata($cat_id)    
	{ 
        $subcats = Categories::where('parent_id',$cat_id)->orderBy('category_name')->get();
		//echo"<pre>";print_r($subcats);echo"</pre>";die();
        return view('pages.ajax_subcat',compact('subcats'));
	//	die();
    } 
	public function ajax_sub_subcatedata($cat_id)    
	{ 
        $subcats = Categories::where('parent_id',$cat_id)->orderBy('category_name')->get();
		//echo"<pre>";print_r($subcats);echo"</pre>";die();
        return view('pages.ajax_sub_subcatedata',compact('subcats'));
	//	die();
    } 
     public function ajax_region($cat_id)    { 
        $regions = Region::where('country_id',$cat_id)->orderBy('region_name')->get();
//echo"<pre>";print_r($regions);echo"</pre>";die();
        return view('pages.ajax_region',compact('regions'));
		die();
    } 
    public function submit_listing()    { 
         ...
         if(!Auth::check())
       {
		...
		//echo "x--".$listing->sub_cat_id."<br>";die();
		//$subcategories = Categories::where('parent_id',$listing->cat_id)->orderBy('category_name')->get();
		 // echo"x--".$listing->cat_id."<br>";
		  
		  
		  //$sub_sub_cates = Categories::where('parent_id',$listing->sub_cat_id)->orderBy('category_name')->get();
       ...
       // $categories = Categories::orderBy('category_name')->get();
		...
		$add=1;
        return view('pages.addeditlisting',compact('categories','locations','countries','regions','add'));
    }
    ...
    { 
        ...
        $listings->cat_id = $inputs['category_id'];
        $listings->sub_cat_id = $inputs['subcat_id'];
		$listings->sub_sub_cat_id = $inputs['sub_subcat_id'];
	   $listings->country_id = $inputs['country_id'];
	   $listings->region_id = $inputs['region_id'];
...
//echo "server-".$_SERVER['REMOTE_ADDR']."<br>";
/*
...
*/
		 ...
		 // echo"x--".$listing->cat_id."<br>";
          ...
          return view('pages.addeditlisting',compact('listing','categories','countries','subcategories','locations','listing_gallery_images','regions','sub_sub_cates','add'));
