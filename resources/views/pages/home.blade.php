...
<div class="spacer feature-section classifieds-content">
  <div class="container">
    ...
    <div class="row">
      <div class="col-md-12">
       @foreach(\App\Categories::where('status',1)->whereNull('parent_id')->orderBy('category_name')->get() as $cat)
      <div class="classified-sec">
        <h1> {{$cat->category_name}}</h1>
        <ul class="categorylisting">
         @foreach(\App\Categories::where('parent_id',$cat->id)->where('status',1)->orderBy('category_name')->get() as $subcat)
          <li>
            <div class="categoryimg">
                <div class="categoryimg_wrap"><a href="">
                   @if(isset($subcat->category_image))
      <!-- <a href="{{URL::to($cat->category_slug.'/'.$cat->id)}}"><img src="{{ URL::asset('upload/category_image/'.$cat->category_image.'-s.jpg') }}" ></a>-->
                 <a class="fancybox fancybox.ajax" href="{{URL::to($cat->category_slug.'/'.$subcat->category_slug.'/'.$subcat->id)}}">   <img src="{{ URL::asset('upload/category_image/'.$subcat->category_image.'-s.jpg') }}" ></a>
                  @endif
                </a></div>
            </div>
            <h2> <a class="fancybox fancybox.ajax" href="{{URL::to($cat->category_slug.'/'.$subcat->category_slug.'/'.$subcat->id)}}">{{$subcat->category_name}}</a></h2>
          </li>
            @endforeach
        </ul>        
      </div>
